...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($input === '') {
        ...
        $stmt = $con->prepare("SELECT id, email FROM admins WHERE email = ? OR contact = ? LIMIT 1");
        ...
        if ($row) {
            ...
            $upd = $con->prepare("UPDATE admins SET reset_token = ?, reset_expires = ? WHERE id = ?");
            ...
            if ($upd->execute()) {
                ...
                if ($mail_ok) {
                    $msg = "A reset link was sent to your email.";
                } else {
                    $msg = "Reset link (mail not configured). Use this link:<br>
