...
if ($token === '') {
    ...
    $stmt = $con->prepare("SELECT id, reset_expiry FROM patients WHERE reset_token = ? LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $err === '') {
    ...
    if ($pass === '' || $pass2 === '') {
        ...
        $upd = $con->prepare("UPDATE patients SET password = ?, reset_token = NULL, reset_expiry = NULL WHERE id = ?");
        $upd->bind_param('si', $hash, $id);
        if ($upd->execute()) {
            $success = "Password updated successfully. You can now <a href='login_patient.php'>login</a>.";
        } else {
...
.reset-card h2 { text-align: center; margin-bottom: 6px; font-size: 24px; font-weight: 700; color: #0f172a; }
.reset-card p { text-align: center; font-size: 14px; margin-bottom: 22px; color: #475569; }
